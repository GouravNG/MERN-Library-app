...
export const registerController = async (req, res) => {
    try {
        ...
        if (errorMessage.includes("E11000")) return res.status(401).json({ "error": "Email entered already exist!" })
        if (errorMessage.includes("User validation failed")) return res.status(401).json({ "error": errorMessage })
